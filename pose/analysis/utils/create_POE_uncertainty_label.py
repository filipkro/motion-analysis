                    ...
                    for idx in label_ind[0]:
                        if str(labels.values[idx, 1]).endswith((leg, '0.0',
                            ...
                            for i in range(5):
                                ...
                                cert_field = action + POE_field + '_uncertainty'
                                dir_field =  action + POE_field + '_dir'
                                label = labels.filter(like=field).values[idx]
                                # uncertainty = labels.filter(like=cert_field).values[idx]
                                if 'mark' not in cohort:
                                    direction = labels.filter(like=dir_field).values[idx]
                                else:
                                    direction = 1
                                # print(label)
                                # print(dir)
                                ...
                                if label.size > 0 and not np.isnan(label):
                                    ...
                                    label_w_uncertainty = 0
                                    # print(label)
                                    if direction == 0:
                                        if label == 0:
                                            label_w_uncertainty = [1]
                                        elif label == 1:
                                            label_w_uncertainty = [2]
                                        elif label == 2:
                                            label_w_uncertainty = [5]
                                        else:
                                            raise Exception(f'incorrect value of label.. \n cohort: {cohort}, subject: {subject}, leg: {leg}, rep: {i}, label: {label}')
                                    elif direction == 1:
                                        if label == 0:
                                            label_w_uncertainty = [0]
                                        elif label == 1:
                                            label_w_uncertainty = [3]
                                        elif label == 2:
                                            label_w_uncertainty = [6]
                                        else:
                                            raise Exception(f'incorrect value of label.. \n cohort: {cohort}, subject: {subject}, leg: {leg}, rep: {i}, label: {label}')
                                    elif direction == 2:
                                        if label == 0:
                                            ...
                                            label_w_uncertainty = [4]
                                        elif label == 2:
                                            label_w_uncertainty = [5]
                                        else:
                                            raise Exception(f'incorrect value of label.. \n cohort: {cohort}, subject: {subject}, leg: {leg}, rep: {i}, label: {label}')
                                    else:
                                        raise Exception(f'incorrect value of dir.. \n cohort: {cohort}, subject: {subject}, rep: {i}, leg: {leg}, dir: {direction}')

                                    dataset_labels.append(label_w_uncertainty)
